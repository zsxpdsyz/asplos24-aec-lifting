...
cva6_su_config = CONFIGS['cva6_su']
SEED = 710
...
def get_model():
    ...
    databasedir = "cva6-model-runs/lsu_traces/su_traces"
    ...
    aul.run_simulate(cva6_su_config, f"{databasedir}/", 10)
    log_timestamp("simulation", CURR_TIME)
    aul.run_genmseq(cva6_su_config, [f"{databasedir}/test_{i}/" for i in range(10)])
    ...
    '''
        Guard synthesis
    '''
    aul.run_consynth(cva6_su_config, databasedir,
    ...
    os.system("time sby -f proofs_lsu.sby taskBMC12_equiv_su")
    os.chdir(cdir)
